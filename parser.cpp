...
void addView(AQLView view) {
    // printf("view.size: %d %d\n", view.columns.size(), view.columns[0].items.size());
	...
	printf("views add View %s at position %d\n\n", view.name.c_str(), views.size()-1);
}
...
void parser(vector<AQLToken> tokens, const string &text, string aqlFile_name) {
	init();
...
//	puts("parser after loadText()");
//	
//	for (int i = 0 ; i < tokens.size(); i++)
//	   printf("%2d %s\n", i, AQLTypeToString(tokens[i].type).c_str());
...
AQLView select_list(vector<AQLToken> tokens, map<string, string> abb) {
	AQLView view;
...
AQLView extract_spec(vector<AQLToken> tokens, map<string, string> abb) {
...
AQLView regex_spec(vector<AQLToken> tokens, map<string, string> abb) {
    puts("extract regex");
...
//	    printf("group %d: %s\n", i, group_name[i].c_str());
//	}

	// promise that the regex are supposed to match ONE WHOLE TextToken
	vector<vector<int> > pos_items = findall(regex.c_str(), textInString.c_str());
	
	// printf("After findall, size: %d X %d\n", pos_items.size(), pos_items[0].size());

	// if (pos_items.empty()) error("The view creating is empty !");
...
// name_spec -> as ID | return group_spec
AQLView pattern_spec(vector<AQLToken> tokens, map<string, string> abb) {
    puts("extract pattern");
...
AQLView pattern_expr(vector<AQLToken> tokens, map<int, string> group_name, map<string, string> abb) {
	set<int> cur_group_num;

	// while (tokens[0].type               == LEFT_ROUND_BRACKET
	// 	&& tokens[tokens.size()-1].type == RIGHT_ROUND_BRACKET) {
			...
			if (tokens[i+1].type == TOKEN) {
				...
				if (i+3 < size && tokens[i+3].type == LEFT_CURLY_BRACKET) {
					int minn = tokens[i+4].num,
						maxx = tokens[i+6].num;
					atoms.push_back(AQLAtom(Token, minn, maxx, cur_group_num));
			...
			// <column> -> <ID.ID>
				...
				if (i+5 < size && tokens[i+5].type == LEFT_CURLY_BRACKET) {
					int minn     = tokens[i+6].num,
						maxx     = tokens[i+8].num;
					atoms.push_back(AQLAtom(Column, abb[viewID], columnID, minn, maxx, cur_group_num));

					i += 9;
				}
				// <ID.ID>
				else {
					atoms.push_back(AQLAtom(Column, abb[viewID], columnID, cur_group_num));
		...
		// /REG/
			...
			if (i+1 < size && tokens[i+1].type == LEFT_CURLY_BRACKET) {
				int minn = tokens[i+1].num,
					maxx = tokens[i+3].num;
				atoms.push_back(AQLAtom(Reg, tokens[i].str, minn, maxx, cur_group_num));

				i += 5;
			}	
			// /REG/
			else {
				atoms.push_back(AQLAtom(Reg, tokens[i].str, cur_group_num));
			}
		}
	}
...
void output_stmt(vector<AQLToken> tokens) {
	...
	// printf("View %s: %d X %d\n", tokens[2].str.c_str(), size_row, size_col);
	...
	for (j = 0; j < size_col; j++) {
		...
		for (i = 0; i < size_row; i++) {
			...
			int len = lengthOfNum(item_st) + lengthOfNum(item_ed) + 4;
			for (k = 0; k < size_items; k++) {
				len += view.columns[j].items[i][k].token.length();
			}

			if (view_name == "Number") printf("size_item: %d\n", size_items);
...
void printItem(const vector<int> &widths, const int &size_col, const AQLView &view) {
	int i, j, k,
		...
		size_items,
		item_st, item_ed;
	// printf("width.size: %d view is %d X %d\n", widths.size(), size_row, size_col);
	string tmp;
	for (i = 0; i < size_row; i++) {
		for (j = 0; j < size_col; j++) {
			tmp = "";
			...
			item_ed = view.columns[j].items[i][size_items-1].ed;
			for (k = 0; k < size_items; k++) {
				tmp += view.columns[j].items[i][k].token;
			}
			tmp += ":(";
